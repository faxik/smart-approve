...
- **Exotic AST nodes no longer bypass rules.** `$(...)`, `<(...)`, heredocs, backticks, `eval`, `source`/`.` are detected (`config.ast_escalate`) and logged as `exotic_escalation`, but rules are tried first on each leaf's FIRST LINE (heredoc bodies are argument content, not command structure); only an unmatched leaf falls to the classifier. See `engine.evaluate` — an earlier version of this bullet claimed a short-circuit that the engine had already dropped.
  ...
  **Known gap in this invariant — tracked as CB-5.** "Rules first, escalate only if unmatched" was reasoned about heredocs, whose bodies really are argument data. `$(...)` is not data — it *executes* — so a matched allow rule carries the substitution with it: `ls $(rm -rf /x)` rule-allows today, even though `command_substitution` is in `ast_escalate`. `engine.py:117` computes `has_exotic` but only passes it out on the unmatched path (line 139); the all-allow return at line 140 drops it, so it gates nothing. Pinned by `test_substitution_in_arguments_is_not_escalated_once_a_rule_matches` so the current behaviour is a decision, not a surprise. Closing it means escalating a matched leaf when its exotic set intersects `ast_escalate`, and splitting that set — heredocs must NOT escalate a rule-allowed leaf, substitutions/eval/source must. Measured cost on a 4834-entry log: 82 of 3758 rule-allows (**2.2%**) would newly reach the classifier, all command substitutions. An earlier version of this note called that a "config-wide latency and cost change" — that was an assumption, and the measurement does not support it. The `git-split-tools` rule opts out locally by rejecting `$` and backticks outright, which is a per-rule workaround, not a fix.
