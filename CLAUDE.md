...
- **Exotic AST nodes no longer bypass rules.** `$(...)`, `<(...)`, heredocs, backticks, `eval`, `source`/`.` are detected (`config.ast_escalate`) and logged as `exotic_escalation`, but rules are tried first on each leaf's FIRST LINE (heredoc bodies are argument content, not command structure); only an unmatched leaf falls to the classifier. See `engine.evaluate` — an earlier version of this bullet claimed a short-circuit that the engine had already dropped.
  ...
  **Known gap in this invariant, open decision.** "Rules first, escalate only if unmatched" was reasoned about heredocs, whose bodies really are argument data. `$(...)` is not data — it *executes* — so a matched allow rule carries the substitution with it: `ls $(rm -rf /x)` rule-allows today, even though `command_substitution` is in `ast_escalate`. Pinned by `test_substitution_in_arguments_is_not_escalated_once_a_rule_matches` so the current behaviour is a decision, not a surprise. Closing it means escalating a matched leaf when its exotic set intersects `ast_escalate`, which sends every `$(…)` command to the classifier — a config-wide latency and cost change, hence not done unilaterally. The `git-split-tools` rule opts out locally by rejecting `$` and backticks outright.
